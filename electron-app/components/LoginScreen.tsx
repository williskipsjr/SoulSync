...
export default function LoginScreen() {
  ...
  const isFakeMode = supabaseHelper.isFake();
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { user, error } = await supabaseHelper.signInWithGoogle();
          ...
          {isFakeMode && (
            <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800">
                ⚠️ <strong>Demo Mode:</strong> Supabase credentials not configured. Using fake authentication.
              </p>
            </div>
          )}
